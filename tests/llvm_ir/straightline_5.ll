...
entry:
  ...
  %circle_area28 = load float, float* %circle_area
  %circle_circum29 = load float, float* %circle_circum
  %addtmp30 = fadd float %circle_area28, %circle_circum29
  %sphere_vol31 = load float, float* %sphere_vol
  %sphere_surf_area32 = load float, float* %sphere_surf_area
  %addtmp33 = fadd float %sphere_vol31, %sphere_surf_area32
  %divtmp = fdiv float %addtmp30, %addtmp33
  store float %divtmp, float* %return_value
  %return_value34 = load float, float* %return_value
  ret float %return_value34
}
